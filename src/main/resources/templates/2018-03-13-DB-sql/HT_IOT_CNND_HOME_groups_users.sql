...
CREATE TABLE `groups_users` (
  ...
  `group_id` varchar(128) NOT NULL COMMENT '유저아이디',
  `user_id` varchar(128) NOT NULL DEFAULT 'uuid' COMMENT '유저아이디',
  `role_id` int(11) NOT NULL DEFAULT '0' COMMENT '권한 아이디',
  `created_time` timestamp NULL DEFAULT NULL,
  `lastmodified_time` timestamp NULL DEFAULT NULL,
  `group_owner_user_id` int(11) DEFAULT NULL,
  `group_gorup_id` varchar(128) DEFAULT NULL,
  `description` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='사용자 그룹';
