...
public class ZWaveCertiNetworkManagementBasicServiceImpl implements ZWaveCertiNetworkManagementBasicService {
	...
	@Autowired
	public ZWaveCertiNetworkManagementBasicServiceImpl(ZWaveRepository zwaveRepository,
			EndpointRepository endpointRepository, CmdClsRepository cmdClsRepository, EndpointService endpointService,
			GatewayRepository gatewayRepository, Properties zWaveProperties, ProducerComponent producerComponent) {
	...
	public void subscribe(ZWaveRequest zwaveRequest, String payload)
		...
		if (zwaveRequest.getCommandKey() == NetworkManagementBasicCommandClass.DEFAULT_SET_COMPLETE
				...
				&& ((int) mqttPayload.getResultData().getOrDefault("status", 255) == 0)) {
			// 해당기기의 정보를 모두 삭제한다.
			zwaveReset(zwaveRequest);
			String topic = callbackAckProperties.getProperty("manager.product.remove");
			String exeTopic = MqttCommon.rtnCallbackAck(topic, Target.app.name(), zwaveRequest.getModel(),
