...
public class HomeServiceImpl implements HomeService {
    ...
    public List<Home> getHomeListByEmail(String authUserEmail, String nickname) {
        ...
        List<ShareHome> shareHomes = shareHomeRepository.findByUserNo(user.getNo());
        List<Integer> nos = new ArrayList();
        
        shareHomes.forEach(ShareHome -> nos.add(ShareHome.getUserNo()));
        
        List<Home> homes = homeRepository.findByNoInAndNicknameContaining(nos, nickname);
    ...
    public boolean shareHome(String mode, Home originHome, User user) {
        ...
        if(Share.share.name().equals(mode)) {
            ShareHome shareHome= new ShareHome(originHome.getNo(), user.getNo(), Share.share.name(), Status.request.name());
        ...
        if(Share.masterModify.name().equals(mode)) {
            ...
            if(!Objects.isNull(user)) {
                ...
                //master를 share로 변경
                ShareHome originShareHome = shareHomeRepository.findByUserNoAndHomeNo(originHome.getOwnerUserNo(), originHome.getNo());
                originShareHome.setRole(ShareRole.share.name());
                shareHomeRepository.save(originShareHome);
                //share를 master로 변경
                ShareHome shareUserGateway = shareHomeRepository.findByUserNoAndHomeNo(originHome.getNo(), user.getNo());
