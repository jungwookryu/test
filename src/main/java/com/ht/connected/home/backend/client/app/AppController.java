package com.ht.connected.home.backend.client.app;
    ...
    public ResponseEntity irDeviceList(@RequestBody HashMap requestMap) {
        ...
        if (rss.getStatusCodeValue() == 200) {
            ...
            if (lstCategory1.size() > 0) {
                ...
                rtnListMap.put("categoryname", IRController.Devicetype.aircon.name());
                rtnListMap.put("list", lstCategory1);
                rtnList.add(rtnListMap);
            }
            if (lstCategory2.size() > 0) {
                HashMap rtnListMap = new HashMap();
                rtnListMap.put("categoryname", IRController.Devicetype.tv.name());
                rtnListMap.put("list", lstCategory2);
                rtnList.add(rtnListMap);
            }
            if (lstCategory3.size() > 0) {
                HashMap rtnListMap = new HashMap();
                rtnListMap.put("categoryname", IRController.Devicetype.fan.name());
